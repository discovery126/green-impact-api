...
WHERE tus_today.task_id IS NULL AND t.type='DAILY' 
   OR tus_any.task_id IS NULL AND t.type='LIMITED' AND t.expired_date > now() AT TIME ZONE 'Europe/Moscow'
""",nativeQuery = true)
...
WHERE tus.completed_at IS NULL  AND tus.user_id=:userId AND (
    tus.taken_at::DATE = CURRENT_DATE AND t.type='DAILY'
        OR t.type='LIMITED'  AND t.expired_date > now() AT TIME ZONE 'Europe/Moscow');""",nativeQuery = true)
